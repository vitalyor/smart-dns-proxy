...
func TestTokenFromSNI(t *testing.T) {
	r := &Router{}
	r.Apply(&model.NodeConfig{DNS: model.DNSConfig{DoTHostname: "dns.example.net"}})
	...
	cases := map[string]string{
		"abc123.dns.example.net":  want("abc123"),
		"ABC123.DNS.EXAMPLE.NET":  want("abc123"), // имя регистронезависимо
		"abc123.dns.example.net.": want("abc123"), // с корневой точкой
		"dns.example.net":         "",             // голое имя резолвера — токена нет
		"":                         "",
		"evil.example.com":         "",             // чужой домен
		"a.b.dns.example.net":     "",             // только один уровень считается токеном
	}
...
func TestTokenFromSNIWithoutHostname(t *testing.T) {
	...
	if got := r.TokenFromSNI("abc.dns.example.net"); got != "" {
