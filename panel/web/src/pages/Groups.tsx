...
export default function Groups({ kind }: { kind: "ingress" | "egress" }) {
  ...
  const title = kind === "ingress" ? "Точки входа" : "Точки выхода";
  const lead = kind === "ingress"
    ? "Группа входных серверов (в РФ), куда устройства отправляют запросы. Сервис получает от неё адреса для управляемых доменов."
    : "Группа выходных серверов (за границей), через которые сервис выходит к сайту. Именно их IP видит конечный сервис.";
    ...
    <>
      ...
      <Notice kind="info" title={lead}>
        {kind === "ingress"
          ...
          : "Локальное переключение выполняет сама входная нода, поэтому резервирование продолжает работать даже при выключенной панели."}
      </Notice>
