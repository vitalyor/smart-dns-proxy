...
export default function Services() {
  ...
  const [wizard, setWizard] = useState<{ preset?: CatalogItem } | null>(null);
  ...
  return (
    <>
      <div className="row">
        ...
        <button className="btn primary" onClick={() => setWizard({})}><IconPlus />Новый сервис</button>
      </div>

      <Notice kind="info" title="Сервис — это то, что вы включаете">
        Выберите готовый сервис из каталога или соберите свой: домены, точка входа и точка выхода —
        всё в одном мастере. Домены живут внутри сервиса; общие «Списки доменов» нужны, только если один
        список переиспользуется несколькими сервисами.
      </Notice>

      {catalog.data && catalog.data.items.length > 0 && (
        <Card title="Каталог" eyebrow="создать в один клик" tight>
          <div className="catalog">
            {catalog.data.items.map((c) => (
              <button key={c.slug} className="catalog-card" onClick={() => setWizard({ preset: c })}>
                <div style={{ fontWeight: 600 }}>{c.name}</div>
                <div className="tiny dim">{c.domains} доменов · проба {c.probe_host}</div>
              </button>
            ))}
          </div>
        </Card>
          ...
          <Card>
            <div className="empty">
              ...
              <p className="muted small">
                Начните с каталога выше или соберите сервис вручную — мастер проведёт по шагам.
              </p>
              <button className="btn primary" style={{ marginTop: 14 }} onClick={() => setWizard({})}>
      ...
      {wizard && (
        <ServiceWizard
          ...
          onSaved={() => { setWizard(null); list.reload(); rules.reload(); }}
        />
      )}
...
function ServiceWizard({ preset, ingress, egress, onClose, onSaved }: {
  preset?: CatalogItem; ingress: Named[]; egress: Named[];
  ...
  const [mode, setMode] = useState<DomainMode>(preset ? "catalog" : "manual");
  const [domains, setDomains] = useState("");
  const [presetKey, setPresetKey] = useState(preset?.preset ?? "");
  ...
  const [probe, setProbe] = useState(preset?.probe_host ?? "");
  ...
  return (
    <Modal title="Новый сервис" onClose={onClose} wide footer={
      ...
      {step === 1 && (
        <>
          <Field label="Название сервиса" hint="Например: Gemini. Идентификатор для метрик создастся автоматически.">
            <input className="input" autoFocus value={name} placeholder="Gemini"
              onChange={(e) => setName(e.target.value)} />
          </Field>
          {catalog.data && catalog.data.items.length > 0 && (
            <Field label="Или выберите из каталога" hint="Подставит название, домены и домен для проверки.">
              <div className="catalog">
                {catalog.data.items.map((c) => (
                  <button key={c.slug} type="button"
                    className={`catalog-card${presetKey === c.preset ? " sel" : ""}`}
                    onClick={() => { setName(c.name); setMode("catalog"); setPresetKey(c.preset); setProbe(c.probe_host); }}>
                    <div style={{ fontWeight: 600 }}>{c.name}</div>
                    <div className="tiny dim">{c.domains} доменов</div>
                  </button>
                ))}
              </div>
            </Field>
