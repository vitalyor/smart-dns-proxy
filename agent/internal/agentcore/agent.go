...
func validate(c *model.NodeConfig, nodeID, role string) error {
	...
	case "egress":
		// Пустой список — законное состояние, а не битая конфигурация: так
		// выглядит нода, которую ещё не назначили ни одному сервису, и нода,
		// которую из всех сервисов убрали. Второй случай важнее: убрав ноду,
		// оператор ждёт, что она перестанет пропускать трафик, а не сохранит
		// прежний список. Реле с пустым списком доменов отказывает всем.
		//
		// Раньше агент такую конфигурацию отвергал, нода вставала в
		// «деградацию» и портила выкат всей ревизии.
	default:
