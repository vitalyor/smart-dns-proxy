...
// handleConnLog proxies the sni-proxy's live connection ring to the panel, with
// the same ?after=<seq> cursor. An egress node has no sni-proxy and reports
// availability:false instead of erroring.
func (a *Agent) handleConnLog(w http.ResponseWriter, r *http.Request) error {
	if a.cfg.ConnLogURL == "" {
		writeJSON(w, http.StatusOK, map[string]any{"available": false, "enabled": false, "seq": 0, "entries": []any{}})
		return nil
	}
	u := a.cfg.ConnLogURL
	if after := r.URL.Query().Get("after"); after != "" {
